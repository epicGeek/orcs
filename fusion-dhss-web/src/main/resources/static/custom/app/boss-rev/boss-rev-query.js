...
function getBossDataDetail(taskId){
	$.ajax({  
        type: "GET",  
        url: "/boss-rev/getBossDataDetail", 
        data: {"taskId":taskId},  
        dataType:"json",  
        success: function(data){
        	...
        	var callbackResult = data.callback_result;
        	if(callbackResult=="success"){
        		callbackResult = "成功";
        	}else if(callbackResult=="failure"){
        		callbackResult = "失败";
        	}
        	var soapLog = data.soap_log;
        	if(soapLog==undefined){
        		soapLog = "";
        	}
        	var errLog = data.err_log;
        	if(errLog==undefined){
        		errLog = "";
        	}
			...
			.kendoGrid(
					{
						...
						columns : [
								...
								{
									field : "response_time",
									template : "<span  title='#:response_time_str#'>#:response_time_str#</span>",
									title : "<span  title='响应时间'>响应时间</span>"
								},
								{
									field : "callback_result",
									template : "<span  title='#:callback_result#'>#:showChineseResult(callback_result)#</span>",
								...
								{
									template : "<button class='queryBtn btn btn-xs btn-info' id='queryDetailButton' onclick=\"getBossDataDetail('#:task_id#')\"><i class='glyphicon glyphicon-edit'></i> 查询详情</button>&nbsp;&nbsp;",
