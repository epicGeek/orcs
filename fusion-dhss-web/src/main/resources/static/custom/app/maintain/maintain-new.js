...
$(function() {
	...
	var inputNeTrigger = $('#inputNeTrigger').kendoDropDownList({
        ...
        change:function(event){
        	...
        	if(gridFilterMap.unitType){
        		filters_checkItem.push({field: "unitType", operator: "contains", value: gridFilterMap["unitType"].replace("_","-")});
            	filters.push({field: "unitType", operator: "eq", value: gridFilterMap["unitType"]});
        	}
        	if(gridFilterMap.neType){
        		filters_checkItem.push({field: "neType", operator: "contains", value: gridFilterMap["neType"].replace("_","-")});
	...
	var unitGrid = $("#unitGrid").kendoGrid({
						...
						height : $(window).height() - $("#unitGrid").offset().top - 50,
						reorderable : true,
						resizable : true,
						sortable : true,
						pageable : false,
						columns: [{
							width: 30,
							template: "<input type='checkbox' id='#:id#' class = 'grid_check_one'/>",
						...
						dataBound: function(){
							...
							$(".grid_check_one").on("click",function(){
								var checkUnit = $("#unitGrid .k-grid-content tbody tr input[type='checkbox']:checked");
								var filters = [];
								$.each(checkUnit,function(index,value){
									var data = unitGrid.dataItem($(value).closest("tr"));
									if(data.unitType){
										filters.push({field: "unitType", operator: "contains", value: data.unitType.replace("_","-")});
									}
								});
	...
	var commandGrid = $("#commandGrid").kendoGrid({
						...
						columns: [{
							...
							attributes:{"class": "text-center"},
							title: "<input type='checkbox' class='command_grid_checkbox'/>"
						},{
							field: "name",
						...
						dataBound: function(){
							//全选按钮
							$(".command_grid_checkbox").on("click",function(){
								$("#commandGrid .k-grid-content input[type='checkbox']").prop("checked",$(this).prop("checked"));
          ...
          transport: {
              ...
              parameterMap: function (options, operation) {
                  if (operation == "read") {
                	  ...
                	  resultparamrs.checkName = $("#checkSelect").val();
    ...
    $("#checkSelect").on("keyup",function(){
    	resultDataSource.read();
    })
      
	//执行结果
	var resultGridObj = $("#resultGrid").kendoGrid({		
		dataSource: resultDataSource,		
		height:$(window).height() - $("#resultGrid").offset().top - 50,		
		reorderable: true,
		resizable: true,
		sortable: true,
		pageable: {
		...
		columns: [{
			...
			template: "#if(isDone == 0){#"+
					  ...
					  "#} else {#"+
					  "完成，<a class='btn btn-link'  onclick='showloadLog(\"#:operationId#\")'>日志</a>"+
	...
	$("#submitBtn").on("click",function(){
		...
		var commandNumber = $("#commandGrid .k-grid-content  input[type='checkbox']:checked").length;
		if($("#unitGrid .k-grid-content input[type='checkbox']:checked").length == 0){
			infoTip({"content":"请选择单元列表！"});
		} else if($("#commandGrid .k-grid-content  input[type='checkbox']:checked").length == 0){
			...
			var checkedItemInput = $("#commandGrid .k-grid-content tbody tr input[type='checkbox']:checked");
		    var unitStr = "";			
			...
			$.each(checkedUnitInput,function(index,value){
				...
				$.each(checkedItemInput,function(index1,value1){					
					...
					data.unitType = data.unitType.replace("_", "-");
					if(data1.unitType.indexOf(data.unitType)!=-1){
						var u = {};
						u.neId = data.neId;
