...
$(function() {
	...
	$("#itemList li").on("click",function(){
		...
		$($(this).attr("target")).removeClass("hide").siblings("div:visible").addClass("hide");
		$("#selectText").html($(this).text());
		
		if($(this).attr("target")=="#project"){
		//	 initTable("project");    //项目的数据
		 usp();
			$("#projects").show();
		}
		if($(this).attr("target")=="#product"){
			initTable("product");    //产品
		}
		if($(this).attr("target")=="#workType"){
			initTable("workType");    //工作类型
		}
		if($(this).attr("target")=="#workContent"){
			initTable("workPackage");    //工作包
		}
		if($(this).attr("target")=="#productPeriod"){
			initTable("stage");    //产品阶段的数据
		}
  ...
  var  tableListObj = {
        ...
        initTable: function() {
            ...
            this.tableObj = $('#dataTables').DataTable({
                ...
                "lengthChange": false,
                "destroy": true,
              //  "data": tableListObj.dataArr,
                 "processing": true,
                 "serverSide": true,
                "ajax": {
            	     "type" : "POST",
            		 "url" : "query/productData",
