...
public class AnalyseUtils {
	...
	public static Map<String,StringBuilder> errorAnalysedDataListToLoader(List<Map<String, Object>> errorAnalysedDataList){
		...
		for (Map<String, Object> errorAnalysedMap : errorAnalysedDataList) {
			try {
				errorJoinLoader.append(errorAnalysedMap.get("task_id")+FIELD_TERMINATOR+errorAnalysedMap.get("err_log")+LINE_TERMINATOR);
			} catch (Exception e) {
	...
	public static Map<String, Object> originalDataTransformer(StringBuilder originalText) {
	...
	public static Map<String, List<Map<String, Object>>> getErrorInfoMap(StringBuilder errorLog)
		...
		for (String normalError : normalErrorListOriginal) {
			try {
				...
				String task_id = normalError.trim().split("={57}")[0].split(" ")[6].trim();
	...
	public static Map<String, List<Map<String, Object>>> errCaseCucLogAnalyser(StringBuilder errCaseUnicomTarget,Map<String,String> TASKID_AND_HLRSN_MAP) throws ParseException, IOException{
		int hlrsnMatched = 0;
		...
		List<Map<String,Object>> insertToBossErrCaseCuc = new ArrayList<>();
		List<Map<String,Object>> insertToBossErrCaseCucTemp = new ArrayList<>();
		List<Map<String,Object>> insertToBossJoin = new ArrayList<>();
		for (String normalErr : normalErrList) {
			//insert to boss_err_case_cuc , boss_err_case_cuc_temp and boss_join
			//boss_err_case field:
			//task_id,response_time(no millisecond),error_code,error_message,operation_name,msisdn,imsi,business_type,mml
			Map<String,Object> dataMap = new HashMap<>();
			Map<String,Object> tempTableDataMap = new HashMap<>();
			...
			String taskId = title.split(":")[title.split(":").length-1].trim();
			...
			joinMap.put("task_id", taskId);
			joinMap.put("err_log", normalErr);
			insertToBossJoin.add(joinMap);
			dataMap.put("task_id", taskId);
			dataMap.put("response_time", responseTimeWithNoMillisecond);
			dataMap.put("mml", request);
			dataMap.put("error_code", errorCode);
			dataMap.put("error_message", errorMessage);
			dataMap.put("operation_name", operation);
			dataMap.put("msisdn", MSISDN);
			dataMap.put("imsi", IMSI);
			dataMap.put("business_type", businessType);
			dataMap.put("hlrsn", hlrsn);
			insertToBossErrCaseCuc.add(dataMap);
			tempTableDataMap.put("task_id", taskId);
			tempTableDataMap.put("response_time", responseTimeWithNoMillisecond);
			tempTableDataMap.put("mml", request);
			tempTableDataMap.put("error_code", errorCode);
			tempTableDataMap.put("error_message", errorMessage);
			tempTableDataMap.put("operation_name", operation);
			tempTableDataMap.put("msisdn", MSISDN);
			tempTableDataMap.put("imsi", IMSI);
			tempTableDataMap.put("business_type", businessType);
			tempTableDataMap.put("hlrsn", hlrsn);
			insertToBossErrCaseCucTemp.add(tempTableDataMap);
		}
		Map<String,List<Map<String,Object>>> analysedUnicomErrCaseMap = new HashMap<>();
		analysedUnicomErrCaseMap.put("boss_err_case_cuc", insertToBossErrCaseCuc);
		analysedUnicomErrCaseMap.put("boss_err_case_cuc_temp", insertToBossErrCaseCucTemp);
		analysedUnicomErrCaseMap.put("boss_join", insertToBossJoin);
		LOGGER.info("HLRSN MATCHED IN THE ERROR LOG:"+hlrsnMatched);
		return analysedUnicomErrCaseMap;
//		for (String requestBlankErr : requestBlankErrCaseList) {  TODO may not useful
//			Map<String,Object> dataMap = new HashMap<>();
//			Map<String,Object> joinMap = new HashMap<>();
//			//insert into boss_err_case_cuc and boss_join
//			//field :task_id,response_time(no millisecond)
//			String title = requestBlankErr.split("={57}")[0].trim();
//			String responseTimeWithNoMillisecond = title.split(",")[0];
//			String responseTimeWithMillisecond = (title.split(" ")[0]+" "+title.split(" ")[1]).replace(",", " ");
//			long taskIdTail = sdf.parse(responseTimeWithMillisecond).getTime();
//			String taskId = title.split(":")[title.split(":").length-1].trim()+"#"+String.valueOf(taskIdTail);
//			
//			joinMap.put("task_id", taskId);
//			joinMap.put("err_log", requestBlankErr.trim());
//			insertToBossJoin.add(joinMap);
//			dataMap.put("task_id", taskId);
//			dataMap.put("response_time", responseTimeWithNoMillisecond);
//		}
