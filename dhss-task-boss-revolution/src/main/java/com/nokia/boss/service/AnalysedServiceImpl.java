...
public class AnalysedServiceImpl implements AnalysedSerivice{ 
	...
	public void soapLogDataAnalysis(File soapLogFile,String soapGwName) throws IOException {//successLogFileName 变化的文件名，是相对路径
		...
		Map<String, Object> soapAnalysedInfo = AnalyseUtils.originalDataTransformer(targetText);//把原始日志转换为LOAD数据
	...
	public void errLogDataAnalysisUnicom(File errLogFile, String soapGwName) throws IOException, ParseException {
		...
		Map<String, List<Map<String, Object>>> m = AnalyseUtils.errCaseCucLogAnalyser(targetText,TASKID_AND_HLRSN_MAP);
		List<Map<String, Object>> insertToBossErrCase = m.get("boss_err_case_cuc");
		List<Map<String, Object>> insertToBossErrCaseTemp = m.get("boss_err_case_cuc_temp");
		List<Map<String, Object>> insertToBossJoin = m.get("boss_join");
		insertIntoDB(insertToBossErrCase, insertToBossErrCaseTemp, insertToBossJoin);
		int endLine = (int)analysisTarget.get("endLine");
		afterLoad("failure", errLogFile.getName(), failureDataFile, soapGwName, endLine);
	}
	public void insertIntoDB(List<Map<String, Object>> insertToBossErrCase,
			List<Map<String, Object>> insertToBossErrCaseTemp, List<Map<String, Object>> insertToBossJoin) {
		NamedParameterJdbcTemplate namedParameterJdbcTemplate = new NamedParameterJdbcTemplate(jdbcTemplate);
		String insertBossErrCase = "insert into boss_err_case_cuc "
				+ "(task_id,response_time,mml,error_code,error_message,operation_name,msisdn,imsi,business_type,hlrsn) "
				+ "	values (:task_id,:response_time,:mml,:error_code,:error_message,:operation_name,:msisdn,:imsi,:business_type,:hlrsn)";
		String insertBossErrCaseTemp = insertBossErrCase.replace("boss_err_case_cuc", "boss_err_case_cuc_temp");
		String insertBossJoin = "insert into boss_join (task_id,err_log)values(:task_id,:err_log)";
		LOGGER.info("EXECUTE SQL for inserting data to boss_err_case_cuc:");
		LOGGER.info(insertBossErrCase);
		for (Map<String, Object> map : insertToBossErrCase) {
			namedParameterJdbcTemplate.update(insertBossErrCase, map);
		}
		LOGGER.info("Complete");
		LOGGER.info("EXECUTE SQL for inserting data to boss_err_case_cuc_temp:");
		LOGGER.info(insertBossErrCaseTemp);
		for (Map<String, Object> map : insertToBossErrCaseTemp) {
			namedParameterJdbcTemplate.update(insertBossErrCaseTemp, map);
		}
		LOGGER.info("Complete");
		LOGGER.info("EXECUTE SQL for inserting data to boss_join:");
		LOGGER.info(insertBossJoin);
		for (Map<String, Object> map : insertToBossJoin) {
			namedParameterJdbcTemplate.update(insertBossJoin, map);
		}
		LOGGER.info("Complete");
	}
	/**
	...
	private void afterLoad(String logType,String filePureName ,File analysedFile,String soapName,int startLine){
		...
		if(filePureName.contains(".gz")){ //ALL GZ
			...
			if(logType.equalsIgnoreCase("failure")){
				if(filePureName.contains(todayStr)){
